  ...
  while($info = mysqli_fetch_array($data))
    {
      ...
      foreach($columnnames as $columnname)
        {
	  if (strlen(utf8_decode($info[$columnname])) < 3 && strlen(utf8_decode($string)) < 3  )
	   {
	     echo "column " .$info[$columnname] . " string $string\n";
	     if (preg_match("/#".$info[$columnname]."(#|\s|$)/i", $string) === 1)
	       {
		 return $info;
	       }
	   }
	  elseif (strlen(utf8_decode($info[$columnname])) == 2 && strlen(utf8_decode($string)) == 2 )
	   {
	     echo "column " .$info[$columnname] . " string $string\n";
	      if (preg_match("/#".$info[$columnname]."(#|\s|$)/i", $string) === 1)
		{
